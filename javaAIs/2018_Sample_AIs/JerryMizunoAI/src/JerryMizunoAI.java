	...
	public synchronized void processing() {
		...
		if(!fd.getEmptyFlag()){	
			if(fd.getRemainingFramesNumber() > 0){
				...
				my = sim_fd.getCharacter(p);
				opp = sim_fd.getCharacter(!p);
							...
							if(calculateActDistance(getOppAttackData(),opp.getX()-my.getX(),opp.getY()-my.getY())){
								...
								for(Iterator<Action> i = myAct.iterator();i.hasNext();){
									...
									for(Iterator<Action> j = oppAct.iterator();j.hasNext();){
										...
										temp_score = (simulatedFrame.getCharacter(p).getHp() - my_original_hp) - (simulatedFrame.getCharacter(!p).getHp() - op_original_hp);
